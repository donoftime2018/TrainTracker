<template>
    ...
    <div class="flex justify-evenly">
        ...
        <div v-if="errMessage !== '' && (trainData === undefined || trainData === [])" class="flex shadow-xl justify-evenly mt-40 text-3xl"><h1>{{errMessage}}</h1></div>

        <div v-else class="flex flex-wrap gap-1 justify-evenly">
            
            <div v-for="t in trainData" class="mt-20 rounded-lg text-center text-white px-6 py-4">
                <div class="shadow-xl divide-y">
                    <div class="flex bg-[#c60c30] p-3 w-full max-w-xl">
                        <div class="flex flex-col"><h2 class="font-bold underline">Parent Station:</h2> <p v-bind:style="{cursor: pointer}" @click="googleMapStation(t.staNm, t.stpDe)">{{t.staNm}}</p></div>
                        <div class="flex flex-col"><h2 class="font-bold underline">Station Description:</h2> <p>{{t.stpDe}}</p></div>
                        <div class="flex flex-col"><h2 class="font-bold underline">Destination Station:</h2> <p v-bind:style="{cursor: pointer}" @click="googleMapStation(t.destNm, t.stpDe)">{{t.destNm}}</p></div>
...
<script>
    ...
    export default {
        ...
        methods: {
            googleMapStation(stationName) {

                let urlParams = stationName + " station, Illinois"

                window.open("https://www.google.com/maps/search/"+urlParams)
            }
        },
        async mounted() {
            console.log("Mounted!")
